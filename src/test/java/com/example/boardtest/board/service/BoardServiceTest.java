...
public class BoardServiceTest {
    ...
    void boardEdit() {
        ...
        // S3Service, boardRepository, imageRepository mock 설정
        given(userRepository.findByLoginId(any())).willReturn(Optional.of(board.getUser()));
        given(boardRepository.findById(board.getId())).willReturn(Optional.of(board));
        given(categoryRepository.findById(category.getId())).willReturn(Optional.of(category));


        // 기존 이미지 목록이 존재한다고 가정 (이미지 삭제되는 로직 검증)
    ...
    void boardFindKeyword() {
        ...
        given(boardRepository.findBoardByTitleContaining(pageable, keyword)).willReturn(page);
